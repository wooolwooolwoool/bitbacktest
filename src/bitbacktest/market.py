...
class Market(ABC):
    ...
    def execute_order(self, kind: Literal['Buy', 'Sell'], quantity: float):
        ...
        self.hist["signals"][kind].append((self.index, price))
        if kind == 'Buy':
            ...
            ret = False
        if ret:
            self.hist["execute_signals"][kind].append((self.index, price))
        return ret

    def add_order(self, kind: Literal['Buy', 'Sell'], quantity: float, price: float):
